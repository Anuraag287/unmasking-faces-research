...
class DataGenerator:
    ...
    def get_files_faces(self):
        ...
        image_files = []
        for dirpath, dirs, files in os.walk('data\lfw-deepfunneled'):
    ...
    def generate_train_images(self, image_size=None, test_image_count=None, train_image_count=None):
        if image_size is None:
            image_size = self.configuration.get('image_size')
        if test_image_count is None:
            test_image_count = self.test_image_count
        ...
        print('Generating training data')
        self.generate_data(self.train_image_count,
                           image_size=image_size,
                           save_to=self.train_data_path)
    ...
    def get_dataset_examples(self, n=5, test_dataset=False):
        """
